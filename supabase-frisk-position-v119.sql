-- RIDEZ v119: Frisk startposition og synlig stilleposition.
-- Gamle Android-systempositioner afvises. En frisk, grov position må vises
-- for fører og følger, men må aldrig påvirke rute, distance eller topfart.
...
declare
  ...
  presence_recorded_at timestamptz;
  presence_lat double precision;
  presence_lng double precision;
  presence_accuracy double precision;
begin
  ...
  for point in select value from jsonb_array_elements(p_points) loop
    ...
    -- Helt gamle positioner fra Androids systemcache må aldrig bruges til den nye tur.
    if lat not between -90 and 90 or lng not between -180 and 180
       or accuracy<0 or accuracy>250
       or rec<ride_created_at-interval '10 seconds' or rec>now()+interval '5 minutes' then
      rejected:=rejected+1;
      continue;
    end if;

    -- Den seneste friske, plausible position må bruges som et STILLE-ikon.
    -- Den går ikke ind i tracking-state og kan derfor ikke give distance eller topfart.
    if rec>=now()-interval '30 seconds'
       and (presence_recorded_at is null or rec>presence_recorded_at) then
      presence_recorded_at:=rec;
      presence_lat:=lat;
      presence_lng:=lng;
      presence_accuracy:=accuracy;
    end if;

    if accuracy>80 or (s.last_recorded_at is not null and rec<=s.last_recorded_at) then
      rejected:=rejected+1;
      continue;
    end if;
  ...
  if accepted>0 then
    ...
    where r.id=rid;
  elsif presence_recorded_at is not null then
    update public.ridez_rides r set
      lat=presence_lat,lng=presence_lng,speed_ms=0,moving=false,
      accuracy_m=presence_accuracy,updated_at=now()
    where r.id=rid and r.active=true;
  end if;

  return jsonb_build_object('accepted',accepted,'rejected',rejected,'stored',inserted,
    'presence',presence_recorded_at is not null,
