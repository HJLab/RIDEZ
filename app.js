...
const state={rideId:null,publicToken:null,driverToken:localStorage.getItem('ridez_driver_token')||null,watchId:null,lastPos:null,lastUpload:0,distanceM:0,moving:false,stoppedSince:null,messagesSeen:new Set(),map:null,marker:null,line:null,points:[],demo:false,demoTimer:null,demoIndex:0,demoBase:null,demoProfile:null,demoRoute:null,demoTravelM:0,demoSimStart:0,mapOrientation:localStorage.getItem('ridez_map_orientation')||'heading'};
...
function bearing(a,b){const rad=x=>x*Math.PI/180,deg=x=>x*180/Math.PI,dLng=rad(b.lng-a.lng),lat1=rad(a.lat),lat2=rad(b.lat);const y=Math.sin(dLng)*Math.cos(lat2),x=Math.cos(lat1)*Math.sin(lat2)-Math.sin(lat1)*Math.cos(lat2)*Math.cos(dLng);return(deg(Math.atan2(y,x))+360)%360}
function initMap(id,rotatable=false){const m=L.map(id,{zoomControl:true,rotate:rotatable,dragRotate:false,touchRotate:false,shiftKeyRotate:false,rotateControl:false}).setView([55.6761,12.5683],8);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'&copy; OpenStreetMap contributors'}).addTo(m);return m}
...
function applyMapOrientation(prev,cur,speed){if(!state.map)return;if(state.mapOrientation==='north'){if(typeof state.map.stopHeadingUp==='function')state.map.stopHeadingUp();if(typeof state.map.setBearing==='function'&&Math.abs(state.map.getBearing?.()||0)>.1)state.map.setBearing(0);return}if(!prev||speed<1||hav(prev,cur)<1)return;const h=bearing(prev,cur);if(typeof state.map.setHeading==='function')state.map.setHeading(h,{ease:state.demo ? 0.45 : 0.22,deadzone:1});else if(typeof state.map.setBearing==='function')state.map.setBearing(-h)}
function setMapOrientation(mode){state.mapOrientation=mode==='north'?'north':'heading';localStorage.setItem('ridez_map_orientation',state.mapOrientation);if(state.mapOrientation==='north'&&state.map){if(typeof state.map.stopHeadingUp==='function')state.map.stopHeadingUp();if(typeof state.map.setBearing==='function')state.map.setBearing(0)}}
...
function resetDriverTripDisplay({clearMarker=true}={}){state.lastPos=null;state.lastUpload=0;state.distanceM=0;state.moving=false;state.stoppedSince=null;state.points=[];state.messagesSeen=new Set();if(state.line&&state.map){state.map.removeLayer(state.line);state.line=null}if(clearMarker&&state.marker&&state.map){state.map.removeLayer(state.marker);state.marker=null}$('speedValue').textContent='0 km/t';$('distanceValue').textContent='0,0 km';$('messageCount').textContent='0';const el=$('messagesList');if(el){el.className='empty';el.textContent='Ingen beskeder endnu.'}}
...
async function createRide(title){state.driverToken=token();state.publicToken=token();localStorage.setItem('ridez_driver_token',state.driverToken);state.rideId=await rpc('ridez_create_ride',{p_driver_token:state.driverToken,p_public_token:state.publicToken,p_title:title});setRideButtons(true);pollMessages()}
...
async function processPosition(cur,speed,accuracy=5){const now=cur.t||Date.now(),prev=state.lastPos;if(prev){const dist=hav(prev,cur);if(dist<1000&&accuracy<80)state.distanceM+=dist}speed=Math.max(0,speed||0);if(speed>=C.MOVING_THRESHOLD_MS){state.stoppedSince=null;setMotion(true)}else if(speed<=C.STOPPED_THRESHOLD_MS){if(!state.stoppedSince)state.stoppedSince=now;if(now-state.stoppedSince>=C.STATIONARY_SECONDS*1000)setMotion(false)}applyMapOrientation(prev,cur,speed);state.lastPos=cur;$('speedValue').textContent=fmtSpeed(speed);$('distanceValue').textContent=`${(state.distanceM/1000).toFixed(1).replace('.',',')} km`;updateMap(cur.lat,cur.lng,true);if(now-state.lastUpload>=C.LOCATION_UPLOAD_MS){state.lastUpload=now;try{await rpc('ridez_update_location',{p_driver_token:state.driverToken,p_lat:cur.lat,p_lng:cur.lng,p_speed_ms:speed,p_moving:state.moving,p_accuracy_m:accuracy})}catch(e){console.error(e)}}}
...
async function initViewer(){$('viewerView').classList.remove('hidden');state.map=initMap('viewerMap');let lastTrackCount=0;async function refresh(){try{const rideResult=await rpc('ridez_public_ride',{p_public_token:publicRideToken});const ride=Array.isArray(rideResult)?rideResult[0]:rideResult;if(!ride){$('viewerStatus').textContent='Turen findes ikke eller er udløbet.';return}$('viewerTitle').textContent=ride.title||'RIDEZ live-tur';$('viewerSpeed').textContent=fmtSpeed(ride.speed_ms);$('viewerUpdated').textContent=ride.updated_at?new Date(ride.updated_at).toLocaleTimeString('da-DK',{hour:'2-digit',minute:'2-digit'}):'–';$('viewerStatus').textContent=ride.active?(ride.moving?'Føreren er på farten':'Motorcyklen holder stille'):'Turen er afsluttet';const mo=$('viewerMotion');mo.textContent=ride.moving?'KØRER':'STILLE';mo.className=`motion ${ride.moving?'moving':'stopped'}`;if(ride.lat!=null)updateMap(ride.lat,ride.lng,true);const pts=await rpc('ridez_public_track',{p_public_token:publicRideToken});if(pts.length!==lastTrackCount){lastTrackCount=pts.length;state.points=pts.map(x=>[x.lat,x.lng]);if(state.line)state.line.setLatLngs(state.points);else if(state.points.length)state.line=L.polyline(state.points,{weight:5,color:'#e11d24'}).addTo(state.map)}}catch(e){console.error(e);$('viewerStatus').textContent='Kunne ikke hente live-data.'}const fast=($('viewerTitle').textContent||'').startsWith('RIDEZ demo');setTimeout(refresh,fast?700:3000)}refresh();$('messageForm').addEventListener('submit',async e=>{e.preventDefault();const name=$('senderName').value.trim(),body=$('messageBody').value.trim();if(!name||!body)return;$('sendFeedback').textContent='Sender…';try{const result=await rpc('ridez_send_message',{p_public_token:publicRideToken,p_sender_name:name,p_body:body});$('messageBody').value='';$('sendFeedback').textContent=result==='moving'?'🏍️ Beskeden er modtaget. Føreren er på farten, så den bliver først vist, når motorcyklen holder stille.':'✓ Beskeden er sendt og kan vises til føreren nu.'}catch(err){$('sendFeedback').textContent='Beskeden kunne ikke sendes. Prøv igen.'}})}
async function initDriver(){$('driverView').classList.remove('hidden');state.map=initMap('driverMap',true);const orientation=$('mapOrientation');if(orientation){orientation.value=state.mapOrientation;orientation.onchange=()=>setMapOrientation(orientation.value)}$('startBtn').onclick=()=>startRide().catch(e=>alert('Kunne ikke starte turen: '+e.message));$('stopBtn').onclick=()=>stopRide();$('shareBtn').onclick=shareRide;$('demoBtn').onclick=()=>{if(state.demo)stopRide();else startDemo().catch(e=>{console.error(e);alert('Kunne ikke starte demo: '+e.message)})}}
