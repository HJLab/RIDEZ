...
function queueElevationSample(cur,now){
  if(state.demo||!state.rideId||!cur||!Number.isFinite(cur.altitude))return;
  if(Number.isFinite(cur.altitudeAccuracy)&&cur.altitudeAccuracy>60)return;
  if(!state.moving&&state.elevationLastQueuedAt)return;
  ...
  const altitude=Number(cur.altitude);
  ...
  state.elevationQueue.push(altitude);state.elevationLastQueuedAt=now;state.elevationLastQueuedPos={lat:Number(cur.lat),lng:Number(cur.lng)};
...
function serializeFunState(){return {samples:state.funSamples.slice(-12000),countryCurrent:state.funCountryCurrent,countryStats:state.funCountryStats,countryBorders:state.funCountryBorders,gpsAccuracySum:state.funGpsAccuracySum,gpsAccuracyCount:state.funGpsAccuracyCount,gpsUnavailableMs:state.funGpsUnavailableMs,offlineMs:state.funOfflineMs,elevationPrev:state.funElevationPrev,ascentM:state.funAscentM,descentM:state.funDescentM,currentClimbM:state.funCurrentClimbM,longestClimbM:state.funLongestClimbM,bumpBuckets:state.funBumpBuckets}}
function restoreFunState(x){if(!x||typeof x!=='object')return;state.funSamples=Array.isArray(x.samples)?x.samples.slice(-12000):[];state.funCountryCurrent=String(x.countryCurrent||'');state.funCountryStats=x.countryStats||{};state.funCountryBorders=+x.countryBorders||0;state.funGpsAccuracySum=+x.gpsAccuracySum||0;state.funGpsAccuracyCount=+x.gpsAccuracyCount||0;state.funGpsUnavailableMs=+x.gpsUnavailableMs||0;state.funOfflineMs=+x.offlineMs||0;state.funElevationPrev=Number.isFinite(+x.elevationPrev)?+x.elevationPrev:null;state.funAscentM=+x.ascentM||0;state.funDescentM=+x.descentM||0;state.funCurrentClimbM=+x.currentClimbM||0;state.funLongestClimbM=+x.longestClimbM||0;state.funBumpBuckets=x.bumpBuckets||{}}
...
async function maybeUpdateFunCountry(cur,now){if(state.demo||state.funCountryBusy||(state.funCountryCurrent&&now-state.funLastCountryAt<1800000&&state.distanceM-state.funLastCountryDistance<50000))return;state.funCountryBusy=true;try{const country=await lookupPhotoCountry(cur);if(country){if(state.funCountryCurrent&&country!==state.funCountryCurrent)state.funCountryBorders++;state.funCountryCurrent=country;state.funCountryStats[country] ||= {distanceM:0,timeMs:0};state.funLastCountryAt=now;state.funLastCountryDistance=state.distanceM}}finally{state.funCountryBusy=false}}
function recordFunSample(cur,speed,accuracy,previous){const now=+cur.t||Date.now(),prev=state.funSamples.at(-1),step=previous?Math.min(1000,hav(previous,cur)):0,dt=prev?Math.max(0,Math.min(120000,now-prev.t)):0;if(Number.isFinite(accuracy)){state.funGpsAccuracySum+=accuracy;state.funGpsAccuracyCount++}if(state.funGpsErrorStartedAt){state.funGpsUnavailableMs+=Math.max(0,now-state.funGpsErrorStartedAt);state.funGpsErrorStartedAt=0}if(state.funCountryCurrent){const c=state.funCountryStats[state.funCountryCurrent] ||= {distanceM:0,timeMs:0};c.distanceM+=step;c.timeMs+=dt}const p={lat:+cur.lat,lng:+cur.lng,t:now,speed:Math.max(0,+speed||0),accuracy:+accuracy||0,d:state.distanceM,day:state.tripDayNumber};if(!prev||now-prev.t>=10000||step>=50||p.day!==prev.day)state.funSamples.push(p);else state.funSamples[state.funSamples.length-1]=p;if(state.funSamples.length>12000)state.funSamples=state.funSamples.filter((_,i)=>i%2===0||i===state.funSamples.length-1);maybeUpdateFunCountry(cur,now).catch(()=>{})}
function recordFunElevation(batch,elev){for(const h0 of elev){const h=+h0;if(!Number.isFinite(h))continue;if(state.funElevationPrev!==null){const d=h-state.funElevationPrev;if(d>=3){state.funAscentM+=d;state.funCurrentClimbM+=d;state.funLongestClimbM=Math.max(state.funLongestClimbM,state.funCurrentClimbM)}else if(d<=-3){state.funDescentM-=d;state.funCurrentClimbM=0}}state.funElevationPrev=h}}
...
function analyzeFunFacts(samples,ride={},x={}){
 ...
 const cs=x.countries||{},names=Object.keys(cs);if(names.length){items.push(funItem(15,names.join(' · ')));items.push(funItem(16,+x.borders||0));items.push(funItem(17,names.map(n=>`${n}: ${(+cs[n].distanceM/1000).toFixed(1).replace('.',',')} km`).join(' · ')));items.push(funItem(18,names.map(n=>`${n}: ${fmtDuration(+cs[n].timeMs/1000)}`).join(' · ')))}
 if(s.length>1){const direct=hav(s[0],s.at(-1)),b=funBearing(s[0],s.at(-1));if(direct>100)items.push(funItem(20,funCompass(b),`${Math.round(b)}°`));items.push(funItem(21,fmtKm(far)));if(direct>100&&distance>direct)items.push(funItem(22,`${(distance/direct).toFixed(1).replace('.',',')}×`,`${fmtKm(distance)} kørt · ${fmtKm(direct)} i luftlinje`));const north=s.reduce((m,p)=>p.lat>m.lat?p:m,s[0]),south=s.reduce((m,p)=>p.lat<m.lat?p:m,s[0]);items.push(funItem(29,`${north.lat.toFixed(4)}° N / ${south.lat.toFixed(4)}° N`,'nordligste / sydligste'));items.push(funItem(30,direct<=Math.max(1000,distance*.05)?'Rundtur':'A til B-tur',`${fmtKm(direct)} mellem start og slut`))}
...
function checkedGpsSpeed(cur,reportedSpeed,accuracy){
  if(!state.lastPos)return {ok:false,reason:'mangler referencepunkt'};
  const dt=(cur.t-state.lastPos.t)/1000;
  if(!(dt>0)||dt>30)return {ok:false,restart:true,reason:'for langt mellem GPS-punkterne'};
  if(!Number.isFinite(accuracy)||accuracy>GPS_SPEED_ACCURACY_M)return {ok:false,reason:'lav GPS-præcision'};
  const dist=hav(state.lastPos,cur),derived=dist/dt,lastAcc=Number(state.lastPos.accuracy)||accuracy;
  const hasReported=Number.isFinite(reportedSpeed),speed=Math.max(0,hasReported?reportedSpeed:derived);
  if(speed>GPS_HARD_MAX_SPEED_MS)return {ok:false,reason:'umulig GPS-hastighed'};
  const positionAllowance=Math.max(8,speed*.45,((accuracy+lastAcc)/Math.max(1,dt))*.45);
  if(hasReported&&speed>12&&Math.abs(speed-derived)>positionAllowance)return {ok:false,reason:'hastighed passer ikke med flyttet afstand'};
  if(speed>20&&derived<2&&dist<Math.max(accuracy,lastAcc))return {ok:false,reason:'hastighed uden reel bevægelse'};
  return {ok:true,speed,derived};
}
function gpsPointAcceptable(cur,accuracy,speed){
  ...
  if(Number(speed)>GPS_HARD_MAX_SPEED_MS)return false;
  if(accuracy>(state.gpsMode==='low'?250:GPS_SPEED_ACCURACY_M))return false;
  ...
  const plausible=dt*GPS_HARD_MAX_SPEED_MS+Math.min(accuracy,50)+Math.min(lastAcc,50);
  if(dist>Math.max(35,plausible))return false;
  return true;
}
async function processPosition(cur,speed,accuracy=5){
  ...
  recordFunSample(cur,speed,accuracy,previousPos);state.currentSpeedMs=speed;const newTop=speed>state.maxSpeedMs+0.05;updateRideStats(now,speed);updateAccelerationStats(now,speed);if(newTop)schedulePublicTopSpeed();
...
async function onPosition(pos){
  ...
  if(!state.demo){
    ...
    const checked=checkedGpsSpeed(cur,speed,cur.accuracy);
    if(!checked.ok){
      if(checked.restart){resetGpsQualityGate();warmUpGps(cur,cur.accuracy)}
    ...
    if(speed>GPS_HIGH_SPEED_MS){
      ...
      if(state.gpsHighSpeedConfirmations<2){
        state.lastPos=cur;state.statsLastT=cur.t;
...
async function initViewer(){
  ...
  async function refresh(){
    try{
      ...
      if(Date.now()-lastViewerFactsAt>15000){lastViewerFactsAt=Date.now();try{renderFunFacts('viewerFunFacts',await rpc('ridez_public_fun_facts_v111',{p_public_token:viewerRideToken}),'Fun Facts kommer frem, når turen har nok målinger.')}catch(e){renderFunFacts('viewerFunFacts',null,'Fun Facts kræver v111-opdateringen.')}}
      ...
      try{
        const galleryPhotos=await rpc('ridez_public_camera_photos_v109',{p_public_token:viewerRideToken,p_after_id:lastViewerGalleryPhotoId});
        if(Array.isArray(galleryPhotos)&&galleryPhotos.length)appendViewerGalleryPhotos(galleryPhotos);
      }catch(galleryErr){console.debug('Billedoversigten kræver v109-databasen',galleryErr)}
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=112').catch(()=>{}));
