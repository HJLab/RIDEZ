...
const params=new URLSearchParams(location.search),publicRideToken=params.get('ride'),followChannelToken=params.get('follow'),demoChannelToken=params.get('demo');
...
function ensureFollowChannelToken(){if(!state.followChannelToken){state.followChannelToken=token();localStorage.setItem('ridez_follow_channel_token',state.followChannelToken)}return state.followChannelToken}
...
function loadVehicleProfiles(){
  try{const raw=JSON.parse(localStorage.getItem('ridez_vehicle_profiles')||'[]');state.vehicleProfiles=Array.isArray(raw)?raw.filter(v=>v&&v.id).map(v=>({...v,type:normalizeVehicleType(v.type),consumptionL100:(Number.isFinite(Number(v.consumptionL100))&&Number(v.consumptionL100)>0?Number(v.consumptionL100):null),tankL:(Number.isFinite(Number(v.tankL))&&Number(v.tankL)>0?Number(v.tankL):null)})):[]}catch(e){state.vehicleProfiles=[]}
...
function createDefaultVehicle(type){const w=vehicleWords(type),v={id:'veh_'+token().slice(0,12),type:w.type,name:w.type==='car'?'Min bil':'Min motorcykel',make:'',model:'',year:null,consumptionL100:null,tankL:null};state.vehicleProfiles.push(v);state.activeVehicleId=v.id;state.preferredVehicleType=v.type;localStorage.setItem('ridez_vehicle_type',v.type);persistVehicleProfiles();return v}
...
function syncVehicleUi(){
  ...
  if(list){
    if(!state.vehicleProfiles.length){list.innerHTML=''}
      ...
      const consumption=Number.isFinite(Number(x.consumptionL100))&&Number(x.consumptionL100)>0?`${Number(x.consumptionL100).toFixed(1).replace('.',',')} l/100 km`:'';const tank=Number.isFinite(Number(x.tankL))&&Number(x.tankL)>0?`${Number(x.tankL).toFixed(1).replace('.',',')} l tank`:'';const details=[x.make,x.model,year,consumption,tank].filter(Boolean).map(escapeHtml).join(' · ');
...
function openVehicleDialog(id=null){
  ...
  if($('vehicleYearInput'))$('vehicleYearInput').value=v&&v.year?v.year:'';if($('vehicleTankInput'))$('vehicleTankInput').value=v&&Number.isFinite(Number(v.tankL))?Number(v.tankL).toFixed(1).replace('.',','):'';if($('vehicleConsumptionInput'))$('vehicleConsumptionInput').value=v&&Number.isFinite(Number(v.consumptionL100))?Number(v.consumptionL100).toFixed(1).replace('.',','):'';
...
function saveVehicle(e){
  if(e&&e.preventDefault)e.preventDefault();const name=($('vehicleNameInput')?.value||'').trim(),type=normalizeVehicleType($('vehicleTypeInput')?.value),make=($('vehicleMakeInput')?.value||'').trim(),model=($('vehicleModelInput')?.value||'').trim(),yearRaw=($('vehicleYearInput')?.value||'').trim(),consumptionRaw=($('vehicleConsumptionInput')?.value||'').trim().replace(',','.'),tankRaw=($('vehicleTankInput')?.value||'').trim().replace(',','.'),fb=$('vehicleFeedback');
  if(!name){if(fb)fb.textContent='Skriv et navn til køretøjet.';return}let year=null;if(yearRaw){year=Number(yearRaw);if(!Number.isInteger(year)||year<1900||year>2100){if(fb)fb.textContent='Årgangen skal være mellem 1900 og 2100.';return}}let consumptionL100=null;if(consumptionRaw){consumptionL100=Number(consumptionRaw);if(!Number.isFinite(consumptionL100)||consumptionL100<0.5||consumptionL100>50){if(fb)fb.textContent='Forbruget skal være mellem 0,5 og 50,0 l/100 km.';return}consumptionL100=Math.round(consumptionL100*10)/10}let tankL=null;if(tankRaw){tankL=Number(tankRaw);if(!Number.isFinite(tankL)||tankL<1||tankL>300){if(fb)fb.textContent='Tankstørrelsen skal være mellem 1 og 300 liter.';return}tankL=Math.round(tankL*10)/10}
  if(state.editingVehicleId){const v=state.vehicleProfiles.find(x=>x.id===state.editingVehicleId);if(v)Object.assign(v,{name,type,make,model,year,consumptionL100,tankL})}
  else{const v={id:'veh_'+token().slice(0,12),name,type,make,model,year,consumptionL100,tankL};state.vehicleProfiles.push(v);if(!state.activeVehicleId)state.activeVehicleId=v.id}
...
function resetElevationState(){if(state.elevationFlushTimer){clearTimeout(state.elevationFlushTimer);state.elevationFlushTimer=null}state.elevationQueue=[];state.elevationRawWindow=[];state.elevationMaxM=null;state.elevationMinM=null;state.elevationLastQueuedAt=0;state.elevationLastQueuedPos=null;state.elevationBusy=false;renderTripExtraSummary()}
function queueElevationSample(cur,now){
  if(state.demo||!state.rideId||!state.moving||!cur||!Number.isFinite(cur.altitude))return;
  const stable=Core.stableElevation?Core.stableElevation(state.elevationRawWindow,cur.altitude,cur.altitudeAccuracy):{accepted:true,value:Number(cur.altitude),values:[Number(cur.altitude)]};
  state.elevationRawWindow=stable.values||state.elevationRawWindow;if(!stable.accepted)return;
  const moved=state.elevationLastQueuedPos?hav(state.elevationLastQueuedPos,cur):Infinity;
  if(state.elevationLastQueuedAt&&now-state.elevationLastQueuedAt<ELEVATION_SAMPLE_MS&&moved<ELEVATION_SAMPLE_M)return;
  const altitude=Number(stable.value);
  if(state.elevationLastQueuedPos&&Number.isFinite(state.elevationLastQueuedPos.altitude)&&Math.abs(altitude-state.elevationLastQueuedPos.altitude)>Math.max(25,moved*.18))return;
  state.elevationMaxM=hasNumber(state.elevationMaxM)?Math.max(Number(state.elevationMaxM),altitude):altitude;
  state.elevationMinM=hasNumber(state.elevationMinM)?Math.min(Number(state.elevationMinM),altitude):altitude;
  recordFunElevation(null,[altitude]);renderTripExtraSummary();
  state.elevationQueue.push(altitude);state.elevationLastQueuedAt=now;state.elevationLastQueuedPos={lat:Number(cur.lat),lng:Number(cur.lng),altitude};
...
async function flushElevationQueue(force=false){
  if(state.elevationBusy||!state.elevationQueue.length||!state.driverToken)return;
  state.elevationBusy=true;if(state.elevationFlushTimer){clearTimeout(state.elevationFlushTimer);state.elevationFlushTimer=null}
  const batch=state.elevationQueue.splice(0,100),driverToken=state.driverToken;
  try{await rpc('ridez_update_elevation_v97',{p_driver_token:driverToken,p_max_elevation_m:state.elevationMaxM,p_min_elevation_m:state.elevationMinM,p_sample_count:batch.length})}
  catch(e){console.warn('Telefonens højdedata kunne ikke gemmes i Supabase',e)}
  finally{state.elevationBusy=false;if(state.elevationQueue.length&&!state.elevationFlushTimer)state.elevationFlushTimer=setTimeout(()=>flushElevationQueue(),force?15000:ELEVATION_FLUSH_MS)}
}
...
const FUN_FACTS_CATALOG={1:['⏱️','Længste stræk uden pause'],2:['☕','Længste pause'],3:['🛑','Pauser over 3 min.'],4:['🌀','Mest snoede 10 km'],5:['↔️','Flest sving til'],6:['⚖️','Svingbalance'],7:['📐','Største hældning'],8:['🚀','Kraftigste acceleration'],9:['🧲','Kraftigste opbremsning'],10:['⛰️','Højeste punkt'],11:['🏞️','Laveste punkt'],12:['⬆️','Samlet stigning'],13:['⬇️','Samlet fald'],14:['🧗','Længste stigning'],15:['🌍','Lande besøgt'],16:['🛂','Grænsekrydsninger'],17:['🗺️','Kilometer pr. land'],18:['⌛','Tid pr. land'],20:['🧭','Hovedretning'],21:['📍','Længst fra start'],22:['〰️','Omvejsfaktor'],23:['⛽','Estimeret brændstof'],24:['🛢️','Tankfulde'],25:['🌗','Dagslys og mørke'],26:['🏁','Længste køredag'],27:['😌','Roligste dag'],28:['⚡','Hurtigste gyldige 10 km'],29:['🧭','Nordligste og sydligste punkt'],30:['🔁','Turtype'],35:['🛰️','Gennemsnitlig GPS-præcision'],36:['📡','Tid uden brugbar GPS'],37:['🌐','Tid uden internet'],39:['🪨','Mest ujævne kilometer'],40:['🔀','Retningsskift']};
const FUN_FACT_IDS=Object.keys(FUN_FACTS_CATALOG).map(Number);
function funItem(id,value,detail=''){return {id,value:String(value??'–').slice(0,120),detail:String(detail||'').slice(0,180)}}
function funBearing(a,b){const r=x=>x*Math.PI/180,d=x=>x*180/Math.PI,y=Math.sin(r(b.lng-a.lng))*Math.cos(r(b.lat)),x=Math.cos(r(a.lat))*Math.sin(r(b.lat))-Math.sin(r(a.lat))*Math.cos(r(b.lat))*Math.cos(r(b.lng-a.lng));return(d(Math.atan2(y,x))+360)%360}
function funAngle(a,b){return Math.abs(((b-a+540)%360)-180)}
function funCompass(b){return ['nord','nordøst','øst','sydøst','syd','sydvest','vest','nordvest'][Math.round(b/45)%8]}
function funDark(t,lat,lng){const d=new Date(t),r=Math.PI/180,day=(Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),d.getUTCDate())-Date.UTC(d.getUTCFullYear(),0,0))/86400000,decl=23.44*r*Math.sin(2*Math.PI*(284+day)/365),h=(d.getUTCHours()+d.getUTCMinutes()/60+lng/15-12)*15*r,e=Math.asin(Math.sin(lat*r)*Math.sin(decl)+Math.cos(lat*r)*Math.cos(decl)*Math.cos(h))/r;return e<-.833}
function resetFunFacts(){Object.assign(state,{funSamples:[],funCountryCurrent:'',funCountryStats:{},funCountryBorders:0,funCountryBusy:false,funLastCountryAt:0,funLastCountryDistance:0,funUnassignedDistanceM:0,funUnassignedTimeMs:0,funGpsAccuracySum:0,funGpsAccuracyCount:0,funGpsUnavailableMs:0,funGpsErrorStartedAt:0,funOfflineMs:0,funOfflineStartedAt:navigator.onLine?0:Date.now(),funElevationPrev:null,funAscentM:0,funDescentM:0,funCurrentClimbM:0,funLongestClimbM:0,funBumpBuckets:{},funLastFactsPublishAt:0,funLastMotionAt:0})}
function serializeFunState(){return {samples:state.funSamples.slice(-12000),countryCurrent:state.funCountryCurrent,countryStats:state.funCountryStats,countryBorders:state.funCountryBorders,unassignedDistanceM:state.funUnassignedDistanceM,unassignedTimeMs:state.funUnassignedTimeMs,gpsAccuracySum:state.funGpsAccuracySum,gpsAccuracyCount:state.funGpsAccuracyCount,gpsUnavailableMs:state.funGpsUnavailableMs,offlineMs:state.funOfflineMs,elevationPrev:state.funElevationPrev,ascentM:state.funAscentM,descentM:state.funDescentM,currentClimbM:state.funCurrentClimbM,longestClimbM:state.funLongestClimbM,bumpBuckets:state.funBumpBuckets}}
function restoreFunState(x){if(!x||typeof x!=='object')return;state.funSamples=Array.isArray(x.samples)?x.samples.slice(-12000):[];state.funCountryCurrent=String(x.countryCurrent||'');state.funCountryStats=x.countryStats||{};state.funCountryBorders=+x.countryBorders||0;state.funUnassignedDistanceM=+x.unassignedDistanceM||0;state.funUnassignedTimeMs=+x.unassignedTimeMs||0;state.funGpsAccuracySum=+x.gpsAccuracySum||0;state.funGpsAccuracyCount=+x.gpsAccuracyCount||0;state.funGpsUnavailableMs=+x.gpsUnavailableMs||0;state.funOfflineMs=+x.offlineMs||0;state.funElevationPrev=Number.isFinite(+x.elevationPrev)?+x.elevationPrev:null;state.funAscentM=+x.ascentM||0;state.funDescentM=+x.descentM||0;state.funCurrentClimbM=+x.currentClimbM||0;state.funLongestClimbM=+x.longestClimbM||0;state.funBumpBuckets=x.bumpBuckets||{}}
function initFunConnectivity(){window.addEventListener('offline',()=>{if(state.rideId&&!state.funOfflineStartedAt)state.funOfflineStartedAt=Date.now()});window.addEventListener('online',()=>{if(state.funOfflineStartedAt){state.funOfflineMs+=Date.now()-state.funOfflineStartedAt;state.funOfflineStartedAt=0}})}
async function maybeUpdateFunCountry(cur,now){
 if(state.demo||state.funCountryBusy||(state.funCountryCurrent&&now-state.funLastCountryAt<10*60*1000&&state.distanceM-state.funLastCountryDistance<10000))return;
 state.funCountryBusy=true;
 try{
  const country=await lookupPhotoCountry(cur);
  if(country){
   if(state.funCountryCurrent&&country!==state.funCountryCurrent)state.funCountryBorders++;
   state.funCountryCurrent=country;const c=state.funCountryStats[country] ||= {distanceM:0,timeMs:0};
   c.distanceM+=state.funUnassignedDistanceM;c.timeMs+=state.funUnassignedTimeMs;state.funUnassignedDistanceM=0;state.funUnassignedTimeMs=0;
   state.funLastCountryAt=now;state.funLastCountryDistance=state.distanceM;
  }
 }finally{state.funCountryBusy=false}
}
function recordFunSample(cur,speed,accuracy,stepM=0,intervalMs=0){
 const now=+cur.t||Date.now(),prev=state.funSamples.at(-1),step=Math.max(0,Number(stepM)||0),dt=Math.max(0,Number(intervalMs)||0);
 if(Number.isFinite(accuracy)){state.funGpsAccuracySum+=accuracy;state.funGpsAccuracyCount++}
 if(state.funGpsErrorStartedAt){state.funGpsUnavailableMs+=Math.max(0,now-state.funGpsErrorStartedAt);state.funGpsErrorStartedAt=0}
 if(state.funCountryCurrent){const c=state.funCountryStats[state.funCountryCurrent] ||= {distanceM:0,timeMs:0};c.distanceM+=step;c.timeMs+=dt}else{state.funUnassignedDistanceM+=step;state.funUnassignedTimeMs+=dt}
 const p={lat:+cur.lat,lng:+cur.lng,t:now,speed:Math.max(0,+speed||0),accuracy:+accuracy||0,d:state.distanceM,day:state.tripDayNumber,segment:state.tripSegmentNumber};
 if(!prev||now-prev.t>=10000||step>=50||p.day!==prev.day||p.segment!==prev.segment)state.funSamples.push(p);else state.funSamples[state.funSamples.length-1]=p;
 if(state.funSamples.length>12000)state.funSamples=state.funSamples.filter((_,i)=>i%2===0||i===state.funSamples.length-1);maybeUpdateFunCountry(cur,now).catch(()=>{})
}
function recordFunElevation(batch,elev){for(const h0 of elev){const h=+h0;if(!Number.isFinite(h))continue;if(state.funElevationPrev!==null){const d=h-state.funElevationPrev;if(d>=5){state.funAscentM+=d;state.funCurrentClimbM+=d;state.funLongestClimbM=Math.max(state.funLongestClimbM,state.funCurrentClimbM)}else if(d<=-5){state.funDescentM-=d;state.funCurrentClimbM=0}}state.funElevationPrev=h}}
function onFunDeviceMotion(e){const now=Date.now();if(!state.rideId||!state.moving||now-state.funLastMotionAt<500)return;state.funLastMotionAt=now;const a=e.acceleration||e.accelerationIncludingGravity;if(!a)return;const v=[+a.x,+a.y,+a.z];if(!v.every(Number.isFinite))return;let m=Math.sqrt(v.reduce((s,x)=>s+x*x,0));if(!e.acceleration)m=Math.abs(m-9.81);const k=String(Math.floor(state.distanceM/1000)),b=state.funBumpBuckets[k] ||= {sum:0,count:0};b.sum+=Math.min(100,m*m);b.count++}
function initFunMotion(){if(!state.funMotionBound&&typeof DeviceMotionEvent!=='undefined'&&typeof DeviceMotionEvent.requestPermission!=='function'){window.addEventListener('devicemotion',onFunDeviceMotion,true);state.funMotionBound=true}}
function analyzeFunFacts(samples,ride={},x={}){
 const s=(samples||[]).map(p=>({lat:+p.lat,lng:+p.lng,t:new Date(p.t||p.created_at).getTime(),speed:+(p.speed??p.speed_ms)||0,d:+p.d,day:+(p.day||p.day_number)||1,segment:+(p.segment||p.segment_number)||1})).filter(p=>Number.isFinite(p.lat)&&Number.isFinite(p.lng)&&Number.isFinite(p.t)).sort((a,b)=>a.t-b.t);let cum=0;s.forEach((p,i)=>{if(!Number.isFinite(p.d)){if(i&&p.day===s[i-1].day&&p.segment===s[i-1].segment)cum+=Math.min(1000,hav(s[i-1],p));p.d=cum}else cum=p.d});const items=[],distance=+(ride.distance_m??x.distanceM??cum)||0,days={},twist={},q=[];let run=0,longRun=0,pause=0,longPause=0,pauses=0,light=0,dark=0,gaps=0,brake=0,changes=0,prevBr=null,fast10=0,far=0;
 for(let i=1;i<s.length;i++){const a=s[i-1],b=s[i],raw=b.t-a.t,dt=Math.max(0,Math.min(120000,raw)),sameSegment=a.day===b.day&&a.segment===b.segment,dist=sameSegment?Math.max(0,Math.min(1000,b.d-a.d)):0,moving=b.speed>1;if(raw>30000)gaps+=raw-30000;if(moving){run+=dt;longRun=Math.max(longRun,run);if(pause>=180000){pauses++;longPause=Math.max(longPause,pause)}pause=0}else{pause+=dt;run=0}if(funDark((a.t+b.t)/2,(a.lat+b.lat)/2,(a.lng+b.lng)/2))dark+=dt;else light+=dt;const day=days[b.day] ||= {d:0,m:0};day.d+=dist;if(moving)day.m+=dt;if(dt>=500&&dt<=10000&&a.speed>3)brake=Math.max(brake,(a.speed-b.speed)/(dt/1000));if(dist>=20){const br=funBearing(a,b);if(prevBr!==null&&funAngle(prevBr,br)>=45){changes++;const k=Math.floor(b.d/10000);twist[k]=(twist[k]||0)+1}prevBr=br}far=Math.max(far,hav(s[0],b));q.push(b);while(q.length&&b.d-q[0].d>10000)q.shift();if(q.length>1&&b.d-q[0].d>=9500)fast10=Math.max(fast10,10000/((b.t-q[0].t)/1000))}
 if(pause>=180000){pauses++;longPause=Math.max(longPause,pause)}if(longRun)items.push(funItem(1,fmtDuration(longRun/1000)));if(longPause)items.push(funItem(2,fmtDuration(longPause/1000)));items.push(funItem(3,pauses));const twistMax=Math.max(0,...Object.values(twist));if(twistMax)items.push(funItem(4,`${twistMax} retningsskift`,'bedste 10 km-stræk'));
 const left=+(ride.turn_left_count??x.left)||0,right=+(ride.turn_right_count??x.right)||0,total=left+right;if(total){items.push(funItem(5,left===right?'lige mange':left>right?'venstre':'højre',`${left} venstre · ${right} højre`));items.push(funItem(6,`${Math.round(left/total*100)} % / ${Math.round(right/total*100)} %`,'venstre / højre'))}const lean=Math.max(+(ride.max_lean_left_deg??x.leanLeft)||0,+(ride.max_lean_right_deg??x.leanRight)||0);if(lean)items.push(funItem(7,`${Math.round(lean)}°`));const accel=x.accel||(+ride.accel_best_80_s>0?{seconds:+ride.accel_best_80_s,startKmh:+ride.accel_best_80_start_kmh,endKmh:+ride.accel_best_80_end_kmh}:null);if(accel)items.push(funItem(8,`${Math.round(accel.startKmh)} → ${Math.round(accel.endKmh)} km/t`,`${(+accel.seconds).toFixed(1).replace('.',',')} sek.`));if(brake>.5)items.push(funItem(9,`${brake.toFixed(1).replace('.',',')} m/s²`,'kun stabile GPS-punkter'));
 const hiRaw=ride.max_elevation_m??x.hi,loRaw=ride.min_elevation_m??x.lo,hi=hiRaw===null||hiRaw===undefined?NaN:+hiRaw,lo=loRaw===null||loRaw===undefined?NaN:+loRaw;if(Number.isFinite(hi))items.push(funItem(10,`${Math.round(hi)} m`));if(Number.isFinite(lo))items.push(funItem(11,`${Math.round(lo)} m`));if(x.ascent>0)items.push(funItem(12,`${Math.round(x.ascent)} m`));if(x.descent>0)items.push(funItem(13,`${Math.round(x.descent)} m`));if(x.climb>0)items.push(funItem(14,`${Math.round(x.climb)} højdemeter`));
 const cs=Object.fromEntries(Object.entries(x.countries||{}).map(([name,value])=>[name,{distanceM:+value.distanceM||0,timeMs:+value.timeMs||0}]));if(x.currentCountry&&x.countryUnassigned){const c=cs[x.currentCountry] ||= {distanceM:0,timeMs:0};c.distanceM+=+x.countryUnassigned.distanceM||0;c.timeMs+=+x.countryUnassigned.timeMs||0}const names=Object.keys(cs);if(names.length){items.push(funItem(15,names.join(' · ')));items.push(funItem(16,+x.borders||0));items.push(funItem(17,names.map(n=>`${n}: ${(+cs[n].distanceM/1000).toFixed(1).replace('.',',')} km`).join(' · ')));items.push(funItem(18,names.map(n=>`${n}: ${fmtDuration(+cs[n].timeMs/1000)}`).join(' · ')))}
 if(s.length>1){const direct=hav(s[0],s.at(-1)),b=funBearing(s[0],s.at(-1)),safeFar=Core.safeFarthestDistance?Core.safeFarthestDistance(distance,far):(far<=distance*1.05+50?far:null);if(direct>100)items.push(funItem(20,funCompass(b),`${Math.round(b)}°`));if(safeFar!==null)items.push(funItem(21,fmtKm(safeFar)));if(direct>100&&distance>=direct*.98)items.push(funItem(22,`${(distance/direct).toFixed(1).replace('.',',')}×`,`${fmtKm(distance)} kørt · ${fmtKm(direct)} i luftlinje`));const north=s.reduce((m,p)=>p.lat>m.lat?p:m,s[0]),south=s.reduce((m,p)=>p.lat<m.lat?p:m,s[0]);items.push(funItem(29,`${north.lat.toFixed(4)}° N / ${south.lat.toFixed(4)}° N`,'nordligste / sydligste'));if(direct<=distance*1.05+50)items.push(funItem(30,direct<=Math.max(1000,distance*.05)?'Rundtur':'A til B-tur',`${fmtKm(direct)} mellem start og slut`))}
 const cons=+(ride.vehicle_consumption_l100??x.consumption),liters=cons>0?distance/1000*cons/100:0;if(liters){items.push(funItem(23,`${liters.toFixed(1).replace('.',',')} liter`,'teoretisk'));if(x.tank>0)items.push(funItem(24,`${(liters/x.tank).toFixed(2).replace('.',',')} tankfulde`,`${(+x.tank).toFixed(1).replace('.',',')} liters tank`))}if(light+dark)items.push(funItem(25,`${Math.round(light/(light+dark)*100)} % dagslys`,`${fmtDuration(light/1000)} lys · ${fmtDuration(dark/1000)} mørke`));const ds=Object.entries(days);if(ds.length){const longest=ds.reduce((a,b)=>b[1].m>a[1].m?b:a);items.push(funItem(26,`Dag ${longest[0]}`,fmtDuration(longest[1].m/1000)));const moving=ds.filter(d=>d[1].m);if(moving.length){const calm=moving.reduce((a,b)=>b[1].d/b[1].m<a[1].d/a[1].m?b:a);items.push(funItem(27,`Dag ${calm[0]}`,`${Math.round(calm[1].d/(calm[1].m/1000)*3.6)} km/t`))}}if(fast10)items.push(funItem(28,`${Math.round(fast10*3.6)} km/t`,'gennemsnit over 10 km'));
 if(x.accCount)items.push(funItem(35,`${Math.round(x.accSum/x.accCount)} m`));items.push(funItem(36,fmtDuration(Math.max(gaps,+x.gpsMissing||0)/1000)));items.push(funItem(37,fmtDuration((+x.offline||0)/1000)));const bs=Object.entries(x.bumps||{}).filter(([,v])=>v.count>=10);if(bs.length){const rough=bs.reduce((a,b)=>b[1].sum/b[1].count>a[1].sum/a[1].count?b:a);items.push(funItem(39,`km ${+rough[0]+1}`,`rystelsesindeks ${Math.sqrt(rough[1].sum/rough[1].count).toFixed(1).replace('.',',')}`))}items.push(funItem(40,changes));return {version:113,items}
}
function currentFunFacts(){const v=activeVehicle();return analyzeFunFacts(state.funSamples,{distance_m:state.distanceM,turn_left_count:state.leftTurnCount,turn_right_count:state.rightTurnCount,max_lean_left_deg:state.maxLeanLeftDeg,max_lean_right_deg:state.maxLeanRightDeg,max_elevation_m:state.elevationMaxM,min_elevation_m:state.elevationMinM,vehicle_consumption_l100:state.rideConsumptionL100},{left:state.leftTurnCount,right:state.rightTurnCount,leanLeft:state.maxLeanLeftDeg,leanRight:state.maxLeanRightDeg,hi:state.elevationMaxM,lo:state.elevationMinM,accel:state.accelBest80,consumption:state.rideConsumptionL100,tank:v?.tankL,countries:state.funCountryStats,currentCountry:state.funCountryCurrent,countryUnassigned:{distanceM:state.funUnassignedDistanceM,timeMs:state.funUnassignedTimeMs},borders:state.funCountryBorders,accSum:state.funGpsAccuracySum,accCount:state.funGpsAccuracyCount,gpsMissing:state.funGpsUnavailableMs+(state.funGpsErrorStartedAt?Date.now()-state.funGpsErrorStartedAt:0),offline:state.funOfflineMs+(state.funOfflineStartedAt?Date.now()-state.funOfflineStartedAt:0),ascent:state.funAscentM,descent:state.funDescentM,climb:state.funLongestClimbM,bumps:state.funBumpBuckets})}
function renderFunFacts(id,payload,empty='Fun Facts måles på næste tur.'){const el=$(id);if(!el)return;const items=(payload?.items||[]).filter(x=>FUN_FACTS_CATALOG[+x.id]);if(!items.length){el.innerHTML=`<div class="empty">${escapeHtml(empty)}</div>`;return}el.innerHTML=`<div class="fun-facts-grid">${items.map(x=>{const m=FUN_FACTS_CATALOG[+x.id];return `<article class="fun-fact-card"><span class="fun-fact-icon">${m[0]}</span><div><small>${escapeHtml(m[1])}</small><strong>${escapeHtml(String(x.value))}</strong>${x.detail?`<p>${escapeHtml(x.detail)}</p>`:''}</div></article>`}).join('')}</div><p class="fun-facts-note">${items.length} af ${FUN_FACT_IDS.length} valgte fakta kunne beregnes ud fra turens målinger.</p>`}
async function publishFunFacts(force=false){if(state.driverToken&&!state.demo)renderFunFacts('driverFunFacts',currentFunFacts(),'Fun Facts kommer frem, når turen har nok målinger.');if(!state.driverToken||state.demo||state.funFactsPublishing||(!force&&Date.now()-state.funLastFactsPublishAt<60000))return;state.funFactsPublishing=true;try{await rpc('ridez_update_fun_facts_v113',{p_driver_token:state.driverToken,p_fun_facts:currentFunFacts()});state.funLastFactsPublishAt=Date.now()}catch(e){console.debug('Fun Facts kræver v113-databasen',e)}finally{state.funFactsPublishing=false}}
...
async function publishLiveStats(){
  ...
  try{
    await rpc(nativeBackgroundUploadEnabled()?'ridez_update_live_stats_v118':'ridez_update_live_stats_v45',{
      p_driver_token:state.driverToken,
...
async function lookupPhotoCountry(pos){
  if(!pos||!Number.isFinite(pos.lat)||!Number.isFinite(pos.lng))return '';
  try{
    const coarseLat=Math.round(Number(pos.lat)*10)/10,coarseLng=Math.round(Number(pos.lng)*10)/10;const url='https://api.bigdatacloud.net/data/reverse-geocode-client?latitude='+encodeURIComponent(coarseLat)+'&longitude='+encodeURIComponent(coarseLng)+'&localityLanguage=da';
    const response=await fetch(url,{method:'GET',cache:'no-store',credentials:'omit'});
    if(!response.ok)return '';
    const data=await response.json();
    return String(data&&data.countryName||'').trim().slice(0,80);
  }catch(e){console.debug('Land kunne ikke fastslås til billedet',e);return ''}
}
async function handleRidePhoto(file,source){
  ...
  try{
    ...
    const countryName=source==='camera'?await lookupPhotoCountry(pos):'';
    ...
    try{await rpc('ridez_register_photo_v109',{p_driver_token:state.driverToken,p_storage_path:path,p_lat:pos.lat,p_lng:pos.lng,p_captured_at:capturedAt.toISOString(),p_day_number:state.tripDayNumber,p_photo_origin:source==='camera'?'camera':'gallery',p_country_name:countryName||null})}
...
function rawRollFromOrientation(e){
  ...
  // beta kan passere 90° i landskab. Klipning ved 90° låste derfor målingen på visse telefonholdere.
  return normalizeDeg(roll);
}
...
async function ensureOrientationPermission(){
  ...
  if(typeof DeviceOrientationEvent.requestPermission==='function'){
    const result=await DeviceOrientationEvent.requestPermission();if(result!=='granted')throw new Error('Adgang til bevægelsessensor blev ikke tilladt.');if(typeof DeviceMotionEvent!=='undefined'&&typeof DeviceMotionEvent.requestPermission==='function'){const motionResult=await DeviceMotionEvent.requestPermission();if(motionResult==='granted'&&!state.funMotionBound){window.addEventListener('devicemotion',onFunDeviceMotion,true);state.funMotionBound=true}}
  }
...
async function calibratePhone(){
  ...
  try{
    ...
    state.calibrationFailed=false;state.leanCalibration=avg;localStorage.setItem('ridez_lean_calibration',String(avg));resetLeanStats();persistActiveRideSession(true);updateCalibrationStatus('Kalibreret ✓');updateCalibrationLive(state.lastRawRoll);renderLeanSummary();
...
function initLeanSensor(){initFunMotion();
...
function persistActiveRideSession(force=false){
  ...
  const payload={rideId:state.rideId,driverToken:state.driverToken,publicToken:state.publicToken,followChannelToken:ensureFollowChannelToken(),startedAt:state.rideStartedAt,ownerToken:ensureOwnerToken(),vehicleId:state.activeVehicleId,distanceM:state.distanceM,pendingUploadDistanceM:state.pendingUploadDistanceM,maxSpeedMs:state.maxSpeedMs,movingMs:state.movingMs,stoppedMs:state.stoppedMs,maxLeanLeftDeg:state.maxLeanLeftDeg,maxLeanRightDeg:state.maxLeanRightDeg,leftTurnCount:state.leftTurnCount,rightTurnCount:state.rightTurnCount,tripLength:state.tripLength,tripDayNumber:state.tripDayNumber,tripSegmentNumber:state.tripSegmentNumber,segmentUploadCount:state.segmentUploadCount,tripStartLocalDate:state.tripStartLocalDate,rideConsumptionL100:state.rideConsumptionL100,elevationMaxM:state.elevationMaxM,elevationMinM:state.elevationMinM,funData:serializeFunState(),savedAt:now};
...
async function resumeInterruptedRide(){
  ...
  try{
    ...
    state.rideId=ride.ride_id||saved.rideId;state.driverToken=saved.driverToken;state.publicToken=ride.public_token||saved.publicToken;state.followChannelToken=saved.followChannelToken||ensureFollowChannelToken();localStorage.setItem('ridez_follow_channel_token',state.followChannelToken);state.rideStartedAt=Number(saved.startedAt)||new Date(ride.created_at).getTime()||Date.now();
    state.distanceM=Math.max(Number(saved.distanceM)||0,Number(ride.distance_m)||0);state.pendingUploadDistanceM=Math.max(0,Number(saved.pendingUploadDistanceM)||0);state.maxSpeedMs=Math.max(Number(saved.maxSpeedMs)||0,Number(ride.max_speed_ms)||0);state.movingMs=Math.max(Number(saved.movingMs)||0,(Number(ride.moving_s)||0)*1000);state.stoppedMs=Math.max(Number(saved.stoppedMs)||0,(Number(ride.stopped_s)||0)*1000);state.rideConsumptionL100=hasNumber(ride.vehicle_consumption_l100)?Number(ride.vehicle_consumption_l100):(hasNumber(saved.rideConsumptionL100)?Number(saved.rideConsumptionL100):null);state.elevationMaxM=hasNumber(ride.max_elevation_m)?Number(ride.max_elevation_m):(hasNumber(saved.elevationMaxM)?Number(saved.elevationMaxM):null);state.elevationMinM=hasNumber(ride.min_elevation_m)?Number(ride.min_elevation_m):(hasNumber(saved.elevationMinM)?Number(saved.elevationMinM):null);restoreFunState(saved.funData);
    ...
    await loadExistingActiveTrack();configureAndroidBackgroundTracking();if(navigator.geolocation)startGpsWatch('high');persistActiveRideSession(true);return true;
...
async function createRide(title){
  ...
  state.driverToken=token();state.publicToken=token();state.followChannelToken=ensureFollowChannelToken();state.rideStartedAt=Date.now();state.rideConsumptionL100=Number.isFinite(Number(v.consumptionL100))&&Number(v.consumptionL100)>0?Number(v.consumptionL100):null;state.elevationMaxM=null;state.elevationMinM=null;state.elevationQueue=[];state.elevationRawWindow=[];state.elevationLastQueuedAt=0;state.elevationLastQueuedPos=null;state.pendingUploadDistanceM=0;state.tripDayNumber=1;state.tripSegmentNumber=1;state.segmentUploadCount=0;state.tripStartLocalDate=localDateKey(state.rideStartedAt);localStorage.setItem('ridez_driver_token',state.driverToken);ensureOwnerToken();
  try{
    const prepared=await rpc('ridez_prepare_follow_channel_v113',{p_owner_token:state.ownerToken,p_suggested_token:state.followChannelToken,p_driver_name:state.userName||'Føreren'});
    if(typeof prepared==='string'&&prepared.length>=32){state.followChannelToken=prepared;localStorage.setItem('ridez_follow_channel_token',prepared)}
    state.rideId=await rpc('ridez_create_ride_v113',{p_owner_token:state.ownerToken,p_driver_token:state.driverToken,p_public_token:state.publicToken,p_follow_channel_token:state.followChannelToken,p_driver_name:state.userName||'Føreren',p_title:title,p_vehicle_type:v.type,p_vehicle_name:v.name||'',p_vehicle_make:v.make||'',p_vehicle_model:v.model||'',p_vehicle_year:v.year||null,p_vehicle_consumption_l100:state.rideConsumptionL100,p_trip_length_code:state.tripLength});
  }catch(e){
    ...
    throw new Error('RIDEZ v113-databasen er ikke aktiveret endnu. Kør supabase-v113.sql én gang.');
  }
...
function warmUpGps(cur,accuracy){
  ...
  if(!Number.isFinite(accuracy)||accuracy>GPS_WARMUP_ACCURACY_M){
    state.gpsWarmupStartedAt=0;state.gpsWarmupGoodPoints=0;state.gpsWarmupLastPos=null;
    $('statusDetail').textContent='GPS klargøres – venter på en præcis position…';
    return false;
  }
  const prev=state.gpsWarmupLastPos;
  if(prev){
    const dt=(cur.t-prev.t)/1000,dist=hav(prev,cur),lastAcc=Number(prev.accuracy)||accuracy;
    const stable=dt>0&&dt<=15&&dist<=Math.max(25,dt*25+accuracy+lastAcc);
    if(!stable){state.gpsWarmupStartedAt=cur.t;state.gpsWarmupGoodPoints=1;state.gpsWarmupLastPos=cur;$('statusDetail').textContent='GPS klargøres – positionen stabiliseres…';return false}
  }
  if(!state.gpsWarmupStartedAt){state.gpsWarmupStartedAt=cur.t;state.gpsWarmupGoodPoints=1}
  else state.gpsWarmupGoodPoints++;
  state.gpsWarmupLastPos=cur;
  const elapsed=Math.max(0,cur.t-state.gpsWarmupStartedAt);
  if(state.gpsWarmupGoodPoints<GPS_WARMUP_MIN_POINTS||elapsed<GPS_WARMUP_MS){
    const left=Math.max(1,Math.ceil((GPS_WARMUP_MS-elapsed)/1000));
    $('statusDetail').textContent=`GPS klargøres – cirka ${left} sek. tilbage…`;
    return false;
  }
  state.gpsReady=true;state.lastPos=cur;state.statsLastT=cur.t;state.gpsHighSpeedConfirmations=0;
  $('statusDetail').textContent='GPS er stabil – turen registreres nu.';
  return false;
}
function checkedGpsSpeed(cur,reportedSpeed,accuracy){
  if(!state.lastPos)return {ok:false,restart:true,reason:'mangler referencepunkt'};
  if(Core.assessGpsSegment)return Core.assessGpsSegment(state.lastPos,cur,reportedSpeed,{maxAccuracyM:state.gpsMode==='low'?250:GPS_SPEED_ACCURACY_M,hardMaxSpeedMs:GPS_HARD_MAX_SPEED_MS,movingThresholdMs:Number(C.MOVING_THRESHOLD_MS||2.5),stoppedThresholdMs:Number(C.STOPPED_THRESHOLD_MS||.8),maxGapMs:state.gpsMode==='low'?90000:15000,maxAccelerationMs2:18,previousSpeedMs:state.currentSpeedMs});
  const dt=(cur.t-state.lastPos.t)/1000,dist=hav(state.lastPos,cur),speed=Number.isFinite(reportedSpeed)?Math.max(0,reportedSpeed):dist/Math.max(.001,dt);return dt>0&&dt<=15&&Number.isFinite(accuracy)&&accuracy<=GPS_SPEED_ACCURACY_M?{ok:true,speedMs:speed,distanceM:speed>=Number(C.MOVING_THRESHOLD_MS||2.5)?dist:0,intervalMs:dt*1000}:{ok:false,restart:dt>15,reason:'GPS-punkt afvist'};
}
function startNewGpsSegment(){state.tripSegmentNumber++;state.segmentUploadCount=0;state.activeSegmentKey=null;state.points=[];state.line=null;persistActiveRideSession(true)}
function registerGpsGap(gapMs){const gap=Math.max(0,Number(gapMs)||0);if(gap>0)state.funGpsUnavailableMs+=gap}
async function processPosition(cur,speed,accuracy=5,acceptedStepM=null,acceptedIntervalMs=null){
  ...
  const step=acceptedStepM===null?(previousPos&&hav(previousPos,cur)<1000?hav(previousPos,cur):0):Math.max(0,Number(acceptedStepM)||0);
  const interval=acceptedIntervalMs===null?(previousPos?Math.max(0,Math.min(now-previousPos.t,state.gpsMode==='low'?90000:15000)):0):Math.max(0,Number(acceptedIntervalMs)||0);
  state.distanceM+=step;state.pendingUploadDistanceM+=step;
  recordFunSample(cur,speed,accuracy,step,interval);state.currentSpeedMs=speed;const newTop=speed>state.maxSpeedMs+0.05,motionBefore=state.moving;updateRideStats(now,speed,interval);updateAccelerationStats(now,speed);if(newTop)schedulePublicTopSpeed();
  ...
  const movingPoint=speed>=Number(C.MOVING_THRESHOLD_MS||2.5),appendMapPoint=movingPoint&&(state.gpsMode!=='low'||now-state.lastLowMapPoint>=GPS_LOW_UPLOAD_MS);updateMap(cur.lat,cur.lng,true,appendMapPoint);if(state.gpsMode==='low'&&appendMapPoint)state.lastLowMapPoint=now;persistActiveRideSession();
  const motionChanged=state.moving!==motionBefore,uploadInterval=(state.gpsMode==='low'||!movingPoint)?GPS_LOW_UPLOAD_MS:C.LOCATION_UPLOAD_MS;
  if(motionChanged||state.segmentUploadCount===0||now-state.lastUpload>=uploadInterval){
    state.lastUpload=now;
    if(nativeBackgroundUploadEnabled()){
      state.pendingUploadDistanceM=0;state.segmentUploadCount++;
      const wallNow=Date.now();if(wallNow-state.lastNativeLiveStatsAt>=5000){state.lastNativeLiveStatsAt=wallNow;await publishLiveStats();publishFunFacts().catch(()=>{})}
      persistActiveRideSession(true);
    }else{
      const uploadDistance=state.pendingUploadDistanceM;try{await rpc('ridez_update_location_v113',{p_driver_token:state.driverToken,p_lat:cur.lat,p_lng:cur.lng,p_speed_ms:speed,p_moving:state.moving,p_accuracy_m:accuracy,p_recorded_at:new Date(now).toISOString(),p_day_number:state.tripDayNumber,p_segment_number:state.tripSegmentNumber,p_step_distance_m:uploadDistance,p_total_distance_m:state.distanceM,p_moving_s:Math.max(0,Math.round(state.movingMs/1000)),p_stopped_s:Math.max(0,Math.round(state.stoppedMs/1000))});state.pendingUploadDistanceM=Math.max(0,state.pendingUploadDistanceM-uploadDistance);state.segmentUploadCount++;await publishLiveStats();publishFunFacts().catch(()=>{});persistActiveRideSession(true)}catch(e){console.error(e);$('statusDetail').textContent='GPS-punktet kunne ikke gemmes i Supabase. RIDEZ prøver igen automatisk.'}
    }
  }
}
async function handlePosition(pos){
  ...
  if(!state.demo){
    ...
    if(state.gpsMode==='low'&&state.lastPos){
      ...
      if(lowModeMoved){resetGpsQualityGate();state.gpsMode='high';setTimeout(()=>startGpsWatch('high'),0);$('statusDetail').textContent='Bevægelse registreret – GPS stabiliseres igen…';return}
    }
    checked=checkedGpsSpeed(cur,speed,cur.accuracy);
    if(!checked.ok){
      if(checked.restart){registerGpsGap(checked.gapMs);startNewGpsSegment();resetGpsQualityGate();warmUpGps(cur,cur.accuracy)}
      state.gpsRejectCount++;
      if(!checked.restart&& (state.gpsRejectCount===1||state.gpsRejectCount%5===0))$('statusDetail').textContent=`GPS-punkt ignoreret: ${checked.reason}. RIDEZ fortsætter automatisk.`;
      return;
    }
    speed=checked.speedMs;
    if(speed>GPS_HIGH_SPEED_MS){
      state.gpsHighSpeedConfirmations++;
      if(state.gpsHighSpeedConfirmations<2){
        state.lastPos=cur;state.statsLastT=cur.t;state.currentSpeedMs=speed;
        $('statusDetail').textContent='Høj GPS-hastighed kontrolleres med næste måling…';
        return;
      }
    }else state.gpsHighSpeedConfirmations=0;
  }else if(state.lastPos&&speed===null){
    const dt=(now-state.lastPos.t)/1000;if(dt>0)speed=hav(state.lastPos,cur)/dt;
  }
  await processPosition(cur,speed||0,cur.accuracy,state.demo?null:checkedGpsDistance(checked),state.demo?null:checked.intervalMs);
}
function checkedGpsDistance(result){return result&&Number.isFinite(Number(result.distanceM))?Math.max(0,Number(result.distanceM)):0}
function onPosition(pos){const job=state.positionQueue.then(()=>handlePosition(pos));state.positionQueue=job.catch(e=>console.error('GPS-punkt kunne ikke behandles',e));return job}
...
async function startDemo(){if(!setupComplete()){openUsernameDialog(true);return}
  ...
  $('demoBadge').textContent=`DEMO v${APP_VERSION}`;$('demoBadge').classList.remove('hidden');
  $('demoBtn').textContent='Stop demo';$('demoBtn').classList.add('active');
  $('rideStatus').textContent='Klargør demo…';
  $('statusDetail').textContent=`Demo v${APP_VERSION} henter din GPS-position og låser rutestarten til en vej højst 120 m væk.`;
  try{
...
async function stopRide(){
  const isFuelDemo=state.demo&&state.demoProfile==='fuel123';
  clearGpsWatch();state.gpsMode='high';stopDemoTimer();
  try{await flushPublicTopSpeed()}catch(e){console.warn(e)}
  const finishedDistance=state.distanceM,fuelDemoMoving=isFuelDemo&&finishedDistance>0?Math.round(finishedDistance/(80/3.6)):null,finishedDuration=fuelDemoMoving??(state.rideStartedAt?Math.max(0,Math.round((Date.now()-state.rideStartedAt)/1000)):0),finishedMoving=fuelDemoMoving??Math.max(0,Math.round(state.movingMs/1000)),finishedStopped=isFuelDemo?0:Math.max(0,Math.round(state.stoppedMs/1000)),finishedMaxSpeed=Math.max(0,state.maxSpeedMs),best80=state.accelBest80,slow80=state.accelSlow80;
  finishTurn(Date.now());if(!state.demo&&state.elevationQueue.length){try{await flushElevationQueue(true)}catch(e){console.warn('Sidste højdepunkter kunne ikke færdigbehandles',e)}}const finishedLeanLeft=Math.max(0,state.maxLeanLeftDeg),finishedLeanRight=Math.max(0,state.maxLeanRightDeg),finishedTurnsLeft=Math.max(0,state.leftTurnCount),finishedTurnsRight=Math.max(0,state.rightTurnCount);try{await publishFunFacts(true)}catch(e){console.warn(e)}
...
async function togglePersistentFollowChannel(){
  if(state.rideId){alert('Afslut turen, før du ændrer følgelinket.');return}
  const status=$('followLinkStatus'),button=$('toggleFollowLinkBtn'),enable=button&&button.dataset.enabled==='0';
  try{const result=await rpc('ridez_set_follow_channel_enabled_v113',{p_owner_token:ensureOwnerToken(),p_enabled:enable});const enabled=result!==false;if(button){button.dataset.enabled=enabled?'1':'0';button.textContent=enabled?'Deaktiver':'Aktiver'}if(status)status.textContent=enabled?'Aktivt':'Deaktiveret'}catch(e){alert('Følgelinket kunne ikke ændres. Prøv igen.')}
}
...
async function selectHistoryDay(dayNumber){
  ...
  try{
    ...
    if(state.historySelectedDay===0){try{const stored=await rpc('ridez_history_fun_facts_v111',{p_owner_token:state.ownerToken,p_ride_id:rideId});renderFunFacts('historyFunFacts',stored?.items?.length?stored:analyzeFunFacts(state.historyTrack,ride,{consumption:ride.vehicle_consumption_l100,hi:ride.max_elevation_m,lo:ride.min_elevation_m}))}catch(e){renderFunFacts('historyFunFacts',analyzeFunFacts(state.historyTrack,ride,{consumption:ride.vehicle_consumption_l100,hi:ride.max_elevation_m,lo:ride.min_elevation_m}))}}
...
async function openHistoryRide(rideId){
  ...
  try{
    const [rows,days,photos]=await Promise.all([
      ...
      rpc('ridez_history_days_v113',{p_owner_token:state.ownerToken,p_ride_id:rideId}).catch(()=>rpc('ridez_history_days_v96',{p_owner_token:state.ownerToken,p_ride_id:rideId})).catch(()=>[]),
...
async function initViewer(){
  ...
  function viewerPhotoDate(row){
    const d=new Date(row.captured_at||Date.now());
    return Number.isFinite(d.getTime())?d:null;
  }
  function renderViewerPhotoGallery(){
    const root=$('viewerPhotoGallery'),countEl=$('viewerPhotoGalleryCount');if(!root)return;
    const rows=Array.from(viewerGalleryPhotos.values()).sort((a,b)=>{
      const ad=Number(a.day_number)||1,bd=Number(b.day_number)||1;
      if(ad!==bd)return ad-bd;
      return String(a.captured_at||'').localeCompare(String(b.captured_at||''));
    });
    if(countEl)countEl.textContent=rows.length===1?'1 billede':`${rows.length} billeder`;
    if(!rows.length){root.innerHTML='<div class="empty">Ingen kamerabilleder delt fra turen endnu.</div>';return}
    const groups=new Map();
    rows.forEach(row=>{const day=Math.max(1,Number(row.day_number)||1);if(!groups.has(day))groups.set(day,[]);groups.get(day).push(row)});
    root.innerHTML=Array.from(groups.entries()).map(([day,items])=>{
      const firstDate=viewerPhotoDate(items[0]);
      const dateText=firstDate?firstDate.toLocaleDateString('da-DK',{weekday:'long',day:'numeric',month:'long',year:'numeric'}):'Dato ikke registreret';
      const countries=[...new Set(items.map(x=>String(x.country_name||'').trim()).filter(Boolean))];
      const countryText=countries.length?countries.join(' · '):'Land ikke registreret';
      const photos=items.map(row=>{
        const url=photoPublicUrl(row.storage_path);if(!url)return '';
        const d=viewerPhotoDate(row),time=d?d.toLocaleTimeString('da-DK',{hour:'2-digit',minute:'2-digit'}):'';
        const country=String(row.country_name||'').trim();
        const caption=[country,time].filter(Boolean).join(' · ')||'Taget med RIDEZ';
        return `<button class="viewer-gallery-photo-open" type="button" data-photo-src="${escapeHtml(url)}" data-photo-caption="${escapeHtml(caption)}"><img src="${escapeHtml(url)}" alt="Kamerabillede fra dag ${day}" loading="lazy"><span>${escapeHtml(caption)}</span></button>`;
      }).join('');
      return `<section class="viewer-photo-day"><div class="viewer-photo-day-heading"><h3>Dag ${day} · ${escapeHtml(dateText)}</h3><span>📍 ${escapeHtml(countryText)}</span></div><div class="viewer-photo-grid">${photos}</div></section>`;
    }).join('');
  }
  function appendViewerGalleryPhotos(rows){
    let changed=false;
    for(const row of (rows||[])){
      const id=Number(row.id);if(!Number.isFinite(id))continue;
      viewerGalleryPhotos.set(id,row);lastViewerGalleryPhotoId=Math.max(lastViewerGalleryPhotoId,id);changed=true;
    }
    if(changed)renderViewerPhotoGallery();
  }
  ...
  async function refresh(){
    try{
      ...
      else setViewerAccess('approved','føreren',true);
  ...
  renderViewerPhotoGallery();
  const viewerPhotoPanel=$('viewerPhotoPanel'),viewerPhotoToggleText=$('viewerPhotoToggleText');
...
document.addEventListener('click',e=>{
  ...
  const galleryBtn=e.target.closest&&e.target.closest('.viewer-gallery-photo-open');
  if(galleryBtn){e.preventDefault();const src=galleryBtn.dataset.photoSrc||'',caption=galleryBtn.dataset.photoCaption||'';if(src)openPhotoViewer(src,caption);return}
...
(publicRideToken||followChannelToken||demoChannelToken)?initViewer().catch(handleInitFailure):initDriver().catch(handleInitFailure);
})();
