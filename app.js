...
function clearGpsWatch(){
  ...
  state.watchId=null;if(ANDROID_NAVIGATION_SAFE_MODE)stopAndroidTrackingForNavigation();
}
function startGpsWatch(mode='high'){
  if(state.demo||!state.rideId||!navigator.geolocation)return;
  if(ANDROID_NAVIGATION_SAFE_MODE){clearGpsWatch();$('rideStatus').textContent='GPS sat på pause';$('statusDetail').textContent='RIDEZ GPS er midlertidigt slået fra for at give Kurviger førsteprioritet.';return}
  clearGpsWatch();
...
async function startRide(){if(ANDROID_NAVIGATION_SAFE_MODE){stopAndroidTrackingForNavigation();alert('RIDEZ GPS er midlertidigt slået fra for at sikre, at Kurviger virker. Historik og de øvrige funktioner kan stadig bruges.');return}if(!setupComplete()){openUsernameDialog(true);return}requestMessageNotificationPermission();if(!navigator.geolocation){alert('GPS understøttes ikke på denne enhed.');return}stopDemoTimer();state.demo=false;$('demoBadge').classList.add('hidden');$('demoBtn').textContent='Start demo';$('demoBtn').classList.remove('active');resetDriverTripDisplay({clearMarker:true});resetSpeedDemoResults(false);await createRide('RIDEZ live-tur');$('rideStatus').textContent='Starter GPS…';startGpsWatch('high')}
