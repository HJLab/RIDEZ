...
function applyMapOrientation(prev,cur,speed){if(!state.map||!prev||speed<1||hav(prev,cur)<1)return;const h=bearing(prev,cur);state.mapOrientation='heading';if(typeof state.map.setHeading==='function')state.map.setHeading(h,{ease:state.demo ? 0.45 : 0.22,deadzone:1});else if(typeof state.map.setBearing==='function')state.map.setBearing(-h)}
