...
function showViewerTopSpeed(ride){
  ...
  if(ride.top_speed_over_limit){value.textContent='Skjult';warning.classList.remove('hidden');if(!state.viewerPenaltyShown){state.viewerPenaltyShown=true;const beacon=warning.querySelector('.blue-beacon');beacon?.classList.remove('flash');void beacon?.offsetWidth;beacon?.classList.add('flash')}return}
  warning.classList.add('hidden');
  if(ride.top_speed_hidden){value.textContent='Afventer fartgrænse';return}
