...
function initFunMotion(){if(typeof DeviceMotionEvent!=='undefined'&&typeof DeviceMotionEvent.requestPermission!=='function')window.addEventListener('devicemotion',onFunDeviceMotion,true)}
...
async function ensureOrientationPermission(){
  ...
  if(typeof DeviceOrientationEvent.requestPermission==='function'){
    const result=await DeviceOrientationEvent.requestPermission();if(result!=='granted')throw new Error('Adgang til bevægelsessensor blev ikke tilladt.');if(typeof DeviceMotionEvent!=='undefined'&&typeof DeviceMotionEvent.requestPermission==='function'){const motionResult=await DeviceMotionEvent.requestPermission();if(motionResult==='granted')window.addEventListener('devicemotion',onFunDeviceMotion,true)}
  }
...
async function stopRide(){
  ...
  try{await flushPublicTopSpeed()}catch(e){console.warn(e)}try{await publishFunFacts(true)}catch(e){console.warn(e)}
...
async function selectHistoryDay(dayNumber){
  ...
  try{
    ...
    if(state.historySelectedDay===0){try{const stored=await rpc('ridez_history_fun_facts_v111',{p_owner_token:state.ownerToken,p_ride_id:rideId});renderFunFacts('historyFunFacts',stored?.items?stored:analyzeFunFacts(state.historyTrack,ride,{consumption:ride.vehicle_consumption_l100,hi:ride.max_elevation_m,lo:ride.min_elevation_m}))}catch(e){renderFunFacts('historyFunFacts',analyzeFunFacts(state.historyTrack,ride,{consumption:ride.vehicle_consumption_l100,hi:ride.max_elevation_m,lo:ride.min_elevation_m}))}}
