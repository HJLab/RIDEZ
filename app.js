...
function getDemoBase(){return new Promise((resolve,reject)=>{if(!navigator.geolocation){reject(new Error('Demo Mode kræver adgang til din placering for at starte på nærmeste vej.'));return}navigator.geolocation.getCurrentPosition(p=>resolve({lat:p.coords.latitude,lng:p.coords.longitude}),()=>reject(new Error('Kunne ikke hente din placering. Tillad placering og prøv igen.')),{enableHighAccuracy:true,timeout:10000,maximumAge:15000})})}
async function snapDemoBaseToRoad(base){
  const url=`https://router.project-osrm.org/nearest/v1/driving/${base.lng},${base.lat}?number=1`;
  const res=await fetch(url,{cache:'no-store'});
  if(!res.ok)throw new Error('Kunne ikke finde den nærmeste vej.');
  const json=await res.json();
  const wp=json.waypoints&&json.waypoints[0];
  if(!wp||!wp.location)throw new Error('Kunne ikke placere demoen på vejnettet.');
  const [lng,lat]=wp.location;
  return{lat,lng,name:wp.name||'nærmeste vej'};
}
...
async function initDriver(){$('driverView').classList.remove('hidden');state.map=initMap('driverMap');$('startBtn').onclick=()=>startRide().catch(e=>alert('Kunne ikke starte turen: '+e.message));$('stopBtn').onclick=()=>stopRide();$('shareBtn').onclick=shareRide;$('demoBtn').onclick=()=>{if(state.demo)stopRide();else startDemo().catch(e=>{console.error(e);alert('Kunne ikke starte demo: '+e.message)})}}
