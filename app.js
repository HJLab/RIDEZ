...
async function loadHistory(){
  ...
  try{
    const rows=await rpc('ridez_history',{p_owner_token:state.ownerToken});
    if(!rows.length){list.className='history-list empty';list.textContent='Ingen afsluttede ture endnu.';return}
    list.innerHTML=rows.map(r=>{
      const avg=Number(r.avg_speed_ms||0)*3.6;
      return `<button class="history-card" data-ride-id="${r.ride_id}"><div><strong>${fmtDate(r.created_at)}</strong><span>${escapeHtml(r.title||'RIDEZ tur')}</span></div><div class="history-summary"><b>${(Number(r.distance_m||0)/1000).toFixed(1).replace('.',',')} km</b><span>${fmtDuration(r.duration_s)}</span><span>Gns. ${avg.toFixed(0)} km/t</span><span>📷 ${Number(r.photo_count||0)}</span></div></button>`
    }).join('');
    list.querySelectorAll('[data-ride-id]').forEach(btn=>btn.addEventListener('click',()=>openHistoryRide(btn.dataset.rideId)));
  }catch(e){console.error(e);list.className='history-list empty';list.textContent='Historik kunne ikke hentes. Kontroller at v16 SQL-opdateringen er kørt i Supabase.'}
}
async function openHistoryRide(rideId){
  ...
  try{
    const [rows,track,photos]=await Promise.all([
      rpc('ridez_history',{p_owner_token:state.ownerToken}),
    ...
    const avg=Number(ride.avg_speed_ms||0)*3.6;
    $('historyDetailTitle').textContent=fmtDate(ride.created_at);
    $('historyDetailMeta').textContent=`${(Number(ride.distance_m||0)/1000).toFixed(1).replace('.',',')} km · ${fmtDuration(ride.duration_s)} · Gns. ${avg.toFixed(0)} km/t`;
...
function closeHistoryRide(){$('historyDetail').classList.add('hidden')}
