...
async function flushElevationQueue(force=false){
  ...
  try{
    ...
    const bmax=Math.max(...elev),bmin=Math.min(...elev);state.elevationMaxM=hasNumber(state.elevationMaxM)?Math.max(Number(state.elevationMaxM),bmax):bmax;state.elevationMinM=hasNumber(state.elevationMinM)?Math.min(Number(state.elevationMinM),bmin):bmin;recordFunElevation(batch,elev);renderTripExtraSummary();
...
async function lookupPhotoCountry(pos){
  ...
  try{
    const url='https://api.bigdatacloud.net/data/reverse-geocode-client?latitude='+encodeURIComponent(pos.lat)+'&longitude='+encodeURIComponent(pos.lng)+'&localityLanguage=da';
...
async function onPosition(pos){
  const receivedAt=Date.now(),recordedAt=Number(pos.timestamp),now=Number.isFinite(recordedAt)&&recordedAt>0?recordedAt:receivedAt,cur={lat:Number(pos.coords.latitude),lng:Number(pos.coords.longitude),t:now,accuracy:Number(pos.coords.accuracy)};
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=111').catch(()=>{}));
